...
export class ServerRemoteCore {
  constructor( socket, manager) {
    ...
    // console.log('new ssid', this.ssid)
  ...
  onSocketMessage(message, isBinary = true) {
    ...
    if( isBinary){
      ...
      switch(msgType){ 
        ...
        //           let mp ,code =0 ;
        //           if( result == 'ok'){
        //             mp = MBP.pack( MB('receiver','8',info))
        //           }else{
        //             mp = MBP.pack( MB('result', info));
        //             code = 0xff
        //           }
        ...
        case BohoMsg.AUTH_REQ:
          if( !this.manager.authManager ) return
          if( this.authState < CLIENT_STATE.SENT_SERVER_READY ){
        ...
        case BohoMsg.AUTH_HMAC:
          if( !this.manager.authManager ) return
          if( this.authState < CLIENT_STATE.SENT_SERVER_NONCE ){
